...
This is where the scripts should be placed
	- WallStick (ReplicatedStorage)
	- Animate (StarterCharacterScripts)
	- ExampleMain (StarterCharacterScripts)
	- WallStickServer (ServerScriptService)
	- WallStickClient (StarterPlayerScripts)
	- RbxCharacterSounds (StarterPlayerScripts)
...
function WallStick.new(player)
	...
	self.SeatedConnection = self.Humanoid.Seated:Connect(function(active)
		self:Destroy()
	end)
...
function WallStick:Destroy()
	...
	--self.Camera:SetCustomMouseLock(false)
	self.CameraUp = UNIT_Y

	self.DeathConnection:Disconnect()
	self.SeatedConnection:Disconnect()
...
function WallStick:OnCollisionStep(dt)
	...
	for _, part in next, parts do
		...
		elseif (part ~= stickPart and not part:IsDescendantOf(self.Character) and not part:IsDescendantOf(self.PhysicsWorld) and part.CanCollide and not PLAYERS:GetPlayerFromCharacter(part.Parent)) then
			local cPart
